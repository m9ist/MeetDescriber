...
- **Пушить ТОЛЬКО по явному запросу или разрешению пользователя** — `git push` сам по себе не выполнять; каждый пуш требует отдельного «пушни»/«запушь» от пользователя
