...
class SpectrumWidget:
    def __init__(self, root: tk.Tk, schedule_fn=None) -> None:
        self._root = root
        ...
        # На Mac обязателен schedule_fn (App._schedule) — root.after() небезопасен:
        # tick 20Hz вместе с NSMenuTrackingSession → SIGABRT _Py_FatalError_TstateNULL.
        # Подробнее: CLAUDE.md → Mac-специфичные решения.
    ...
    def show(self) -> None:
        self._schedule(self._create_window)

    def hide(self) -> None:
        self._schedule(self._destroy_window)
    ...
    def _create_window(self) -> None:
        ...
        # На Mac «-topmost» работает только внутри Python-приложения.
        # Чтобы спектр был поверх Chrome и других апп — поднимаем NSWindow level.
        if config.IS_MAC:
            self._raise_above_apps_mac(win)

        self._tick()

    def _raise_above_apps_mac(self, win: tk.Toplevel) -> None:
        """Устанавливает NSWindow level в floating — выше окон других приложений."""
        try:
            ...
            for nswin in NSApp.windows():
                ...
                if int(fr.size.width) == w and int(fr.size.height) == h:
                    nswin.setLevel_(NSFloatingWindowLevel)
                    return
            log.warning("spectrum: NSWindow %dx%d не найден среди NSApp.windows()", w, h)
        except Exception:
            log.error("spectrum NSWindow level error:\n%s", traceback.format_exc())
    ...
    def _tick(self) -> None:
        ...
        self._schedule(self._tick, UPDATE_MS)
